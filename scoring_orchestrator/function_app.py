...
# --------- Digital Readiness latest (live from ADX) ---------
@app.function_name("digital_readiness_latest_live")
@app.route(
    route="digital_readiness_latest_live",
    methods=["GET"],
    auth_level=func.AuthLevel.ANONYMOUS,
)
def digital_readiness_latest_live(req: func.HttpRequest) -> func.HttpResponse:
    """
    GET /api/digital_readiness_latest_live?customer=&participant=
    Returns the latest Digital Readiness record for a given customer / participant
    in a dial-friendly format.
    """
    try:
        customer = (req.params.get("customer") or "").strip()
        participant = (req.params.get("participant") or "").strip()
...
| top 1 by todatetime(timestamp) desc
| project customer,
          participant_name,
          timestamp,
          total_140,
...
"""
        ...
        row0 = table.rows[0]
        score = int(row0["total_140"] or 0)
        percent = float(row0["percent"] or 0.0)
        level = int(row0["level"] or 0)
        maturity = str(row0["maturity"] or "")
        ts = str(row0["timestamp"])
        ...
        payload = {
            ...
            "level": level,        # 1–5
            "maturity": maturity,  # Not Started / Initial / Developing / Advanced / Leading
            "timestamp": ts,
        }
        return func.HttpResponse(
            json.dumps(payload),
        ...
        logging.exception("digital_readiness_latest_live error")
        return func.HttpResponse(
            json.dumps({"error": str(ex)}),
            status_code=500,
            mimetype="application/json",
