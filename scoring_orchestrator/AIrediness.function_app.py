...
# NEW: AI Readiness table + mapping (update defaults if needed)
...
# --------- NEW: AI Readiness helpers (simple flat row) ---------
def _validate_ai_readiness(body: Dict[str, Any]) -> None:
    required = [
        ...
        "answers", "total_105", "percent", "level", "maturity", "notes"
    ]
...
# --------- NEW: AI Readiness ingest endpoint ---------
@app.function_name("ai_readiness_score_and_push")
@app.route(route="ai_readiness_score_and_push", methods=["POST"], auth_level=func.AuthLevel.FUNCTION)
def ai_readiness_score_and_push(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if not KUSTO_INGEST_URI:
        return func.HttpResponse("Accepted (no KUSTO_INGEST_URI; AI row built)", status_code=200)
    ...
    try:
        ...
        resp = requests.post(ingest_url, headers=headers, data=event_bytes, timeout=30)
        if resp.status_code not in (200, 202):
            logging.error("AI ADX ingest failed: %s %s", resp.status_code, resp.text)
            return func.HttpResponse(f"AI ADX ingest error: {resp.text}", status_code=502)
        return func.HttpResponse("Accepted", status_code=200)
    except Exception as e:
